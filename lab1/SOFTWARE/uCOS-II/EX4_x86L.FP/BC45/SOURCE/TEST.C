...
void main(void) {
  ...
  OSTaskCreateExt(TaskStart, (void *)0, &TaskStartStk[TASK_STK_SIZE - 1], 0, /* Task priority = 0 */
                  0, &TaskStartStk[0], TASK_STK_SIZE, (void *)0, OS_TASK_OPT_SAVE_FP);
...
static void TaskStartDispInit(void) {
  ...
  PC_DispStr(0, 2, "                                                                                ",
             DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
  PC_DispStr(0, 3, "                                    EXAMPLE #4                                  ",
             DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
  PC_DispStr(0, 4, "                                                                                ",
             DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
  PC_DispStr(0, 5, "TaskPrio      Angle   cos(Angle)   sin(Angle)                                   ",
             DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
  PC_DispStr(0, 6, "--------      -----   ----------   ----------                                   ",
             DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
  PC_DispStr(0, 7, "                                                                                ",
...
static void TaskStartCreateTasks(void) {
  ...
  for (i = 0; i < N_TASKS; i++) { /* Create N_TASKS identical tasks           */
    prio = i + 1;
    TaskData[i] = prio;
    OSTaskCreateExt(Task, (void *)&TaskData[i], &TaskStk[i][TASK_STK_SIZE - 1], prio, 0, &TaskStk[i][0], TASK_STK_SIZE,
                    (void *)0, OS_TASK_OPT_SAVE_FP);
  }
}
